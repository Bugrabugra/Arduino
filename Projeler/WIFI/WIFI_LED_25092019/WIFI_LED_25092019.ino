...
void setup() {
  ...
  while (!Serial.find("OK"))
    ;                                                            //Ayarlamanın tamamlanmasını bekliyoruz.
  Serial.println("AT+CWJAP=\"" + ssid + "\",\"" + pass + "\"");  //Wifi'a bağlanıyoruz.
  delay(6000);
  while (!Serial.find("OK"))
    ;                               //Bağlanana kadar bekliyoruz.
...
void loop() {
  if (Serial.available()) {      //Seri haberleşme verisi geldiğinde içindeki komutlar çalışır.
    if (Serial.find("+IPD,")) {  //Bağlantı isteği geldiğinde içindeki komutlar çalışır.
      ...
      if (komut.indexOf("yak") != -1) {  //İleri komutu geldiğinde içerisindeki komutlar çalışır.
        digitalWrite(pinLED, HIGH);
      } else if (komut.indexOf("sondur") != -1) {  //Geri komutu geldiğinde içerisindeki komutlar çalışır.
