...
bool event1wasPlayed = false; // flag variables
...
void loop() {
 roboEyes.update(); // update eyes drawings
  ...
  if(millis() >= eventTimer+2000 && event1wasPlayed == 0){
    event1wasPlayed = true; // flag variable to make sure the event will only be handled once
    roboEyes.open(); // open eyes
  }
  // Do once after defined number of milliseconds
  if(millis() >= eventTimer+4000 && event2wasPlayed == 0){
    event2wasPlayed = true; // flag variable to make sure the event will only be handled once
    roboEyes.setMood(HAPPY);
    //roboEyes.anim_confused();
  }
  // Do once after defined number of milliseconds
  if(millis() >= eventTimer+6000 && event3wasPlayed == 0){
    event3wasPlayed = true; // flag variable to make sure the event will only be handled once
